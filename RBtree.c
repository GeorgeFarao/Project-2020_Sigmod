...
void postorder_initialize_bow_tfidf_row(struct RBTree *Tree, struct node *node, HashTable *different_words)
{
    ...
    if (node != Tree->NIL)
    {
        postorder_initialize_bow_tfidf_row(Tree, node->left, different_words);
        postorder_initialize_bow_tfidf_row(Tree, node->right, different_words);
    }

    node->bow = malloc(sizeof(int) * different_words->total_words);
    node->tf_idf = malloc(sizeof(double) * different_words->total_words);

    memset(node->bow, 0, sizeof(int) * different_words->total_words);
    memset(node->tf_idf, 0, sizeof(double) * different_words->total_words);

    traversing_words_for_tfidf_bow(different_words, node);
}
...
void postorder_help_to_initialize_bow_tfidf_row(struct RBTree *Tree, struct node *recursion_root, struct node *file, HashTable *different_words)
{
    ...
    {
        postorder_help_to_initialize_bow_tfidf_row(Tree, recursion_root->left, file, different_words);
        postorder_help_to_initialize_bow_tfidf_row(Tree, recursion_root->right, file, different_words);
    }

    file->bow[recursion_root->word_id] = recursion_root->bow[file->bow_tf_idf_index];

    double tf, idf;
    tf = file->bow[recursion_root->word_id] / file->number_of_words;
    idf = log(global_index / recursion_root->tfidf_nt);

    file->tf_idf[recursion_root->word_id] = tf * idf;
}